...
public class HandlerPagarComCreditos extends HandlerBotDelivery {
    ...
    protected boolean runFirstTime(Message m) {
        if (((ChatBotDelivery)chat).getCliente().getCreditosDisponiveis() >= ((ChatBotDelivery)chat).getPedidoAtual().getTotal()) {
            if (((ChatBotDelivery)chat).getCliente().getCreditosDisponiveis() > ((ChatBotDelivery)chat).getPedidoAtual().getTotal()) {
                ((ChatBotDelivery)chat).getCliente().setCreditosDisponiveis(((ChatBotDelivery)chat).getCliente().getCreditosDisponiveis() - ((ChatBotDelivery)chat).getPedidoAtual().getTotal());
            } else {
                ((ChatBotDelivery)chat).getCliente().setCreditosDisponiveis(0);
            }
