    ...
    protected boolean runFirstTime(Message m) {
        if (!Configuracao.getInstance().isOpenPedidos()) {
            if (Configuracao.getInstance().isAbrirFecharPedidosAutomatico()) {
                chat.getChat().sendMessage("Não iniciamos o atendimento ainda, nosso horário de atentimento é das " + Configuracao.getInstance().getHoraAutomaticaAbrirPedidos().format(DateTimeFormatter.ofPattern("HH:mm")) + " às " + Configuracao.getInstance().getHoraAutomaticaFecharPedidos().format(DateTimeFormatter.ofPattern("HH:mm")) + ", porém você pode agendar o horario do seu pedido.", 1000);
            } else {
