...
public class HandlerSolicitarHorarioReserva extends HandlerBotDelivery {
    ...
    protected boolean runSecondTime(Message m) {
        ...
        try {
            ...
            if (((dataInformada.get(Calendar.HOUR_OF_DAY) == Configuracao.getInstance().getHoraInicioReservas().getHour() && dataInformada.get(Calendar.MINUTE) >= Configuracao.getInstance().getHoraInicioReservas().getMinute()) || dataInformada.get(Calendar.HOUR_OF_DAY) > Configuracao.getInstance().getHoraInicioReservas().getHour()) && ((dataChat.get(Calendar.HOUR_OF_DAY) == horaAtual.get(Calendar.HOUR_OF_DAY) && dataChat.get(Calendar.MINUTE) > horaAtual.get(Calendar.MINUTE)) || dataChat.get(Calendar.HOUR_OF_DAY) > horaAtual.get(Calendar.HOUR_OF_DAY) || DateUtils.isAfterDay(((ChatBotDelivery)chat).getReservaAtual().getDataReserva(), Calendar.getInstance().getTime()))) {
    ...
    protected void onError(Message m) {
        ...
        chat.getChat().sendMessage("*Obs²*: Os horarios de reservas disponíveis são apenas para após as *"+Configuracao.getInstance().getHoraInicioReservas().format(DateTimeFormatter.ofPattern("HH:mm"))+"*");
